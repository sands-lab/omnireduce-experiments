...
while [ $i -lt $wnum ]
do
    ssh -p 2222 ${worker_arr[$i]} "cd /home/exps/models/LSTM; mkdir -p ./100G-results/omnireduce/ ; export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}; export GLOO_SOCKET_IFNAME=${GLOO_SOCKET_IFNAME}; export OMPI_COMM_WORLD_SIZE=${wnum}; export OMPI_COMM_WORLD_RANK=$i; export OMPI_COMM_WORLD_LOCAL_RANK=0; export PATH=$PATH:/usr/local/conda/bin; nohup ./run.sh  --init tcp://${worker_arr[0]}:4000 --backend gloo > ./100G-results/omnireduce/log.txt 2>&1 &"
    i=$((i+1))
done
# check completed
while [[ 1 ]]
